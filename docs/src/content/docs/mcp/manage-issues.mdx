---
title: manage_issues
description: MCP tool for Jira issue operations — get, create, update, assign, transition, comment, delete, link, move, watchers, and more.
---
...
<Aside type="caution">
Moving issues is only supported for **company-managed (classic)** projects. Team-managed (next-gen) projects do not support cross-project moves via the Jira REST API.
</Aside>
...
<Aside type="tip">
Descriptions and comments accept **markdown** which is automatically converted to Jira's ADF (Atlassian Document Format). Supported: headings, bold, italic, strikethrough, links, bullet and numbered lists, blockquotes, tables, code blocks, and horizontal rules.
</Aside>

<Aside type="note">
Write actions are only available if the API token has the corresponding Jira permission. The tool description dynamically reflects available actions based on permission introspection at startup.
</Aside>
