...
<Aside type="tip">
jtk's MCP tools use an action-based multiplexing pattern. Instead of registering 40+ individual tools, jtk groups related operations into 11 tools (e.g., `manage_issues` handles `get`, `create`, `update`, `transition`, etc.). This keeps the tool list manageable for AI agents.
</Aside>
